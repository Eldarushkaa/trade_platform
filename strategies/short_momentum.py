...
Тренд-фильтр (контекст):
    ...
    DI- > DI+                   — медвежье направление подтверждено
...
Momentum-триггер (сигнал входа):
    ...
    4. Bear close: (close - low) / (high - low) < BEAR_CLOSE_RATIO  — закрытие в нижней части

Anti-flush защита (rolling window):
    Если ≥ MAX_IMPULSE_STREAK импульсных медвежьих свечей из IMPULSE_WINDOW последних
    → вход блокируется. Цель: не входить в финальный flush/cascade.
...
Fitness (compute_fitness override):
    ...
    Sharpe 10% — защита от переобучения на одном big win.
...
class ShortMomentumBot(BaseStrategy):
    ...
    RSI_EXIT           = 58    # Выход если RSI > RSI_EXIT (58 — даём импульсу развиться)
    VOL_RATIO_MIN      = 1.0   # Мин. ATR/EMA_ATR для входа (фильтр мёртвого рынка)
    BEAR_CLOSE_RATIO   = 0.35  # Макс. (close-low)/(high-low) для медвежьего закрытия
    MAX_IMPULSE_STREAK = 3     # Блокировка входа после N импульсных из M последних свечей
    IMPULSE_WINDOW     = 5     # Размер окна M для rolling anti-flush
    MAX_HOLD_CANDLES   = 48    # Time-stop (48 × 5m ≈ 4 ч)
    COOLDOWN_CANDLES   = 4     # Мин. свечей между сделками (~20 мин на 5m)
    TRADE_FRACTION     = 1.0   # Доля баланса на сделку
    ...
    def compute_fitness(
        ...
        Целевые метрики:
          - Return:        30% (реальная прибыль важна)
          - Profit Factor: 30% (качество направленности)
          - Drawdown:      20% (momentum DDs бывают резкими)
          - Sharpe:        10% (консистентность — защита от одного big win)
    ...
    PARAM_SCHEMA = {
        ...
        "RSI_EXIT": {
            "type": "float", "default": 58.0, "min": 40.0, "max": 70.0,
            "description": "Выход из SHORT если RSI > RSI_EXIT (импульс иссяк)",
            "optimize": False,
        },
        "VOL_RATIO_MIN": {
            "type": "float", "default": 1.0, "min": 0.5, "max": 2.0,
            "description": "Мин. ATR/EMA_ATR для входа (фильтр мёртвого рынка)",
            "optimize": False,
        },
        "BEAR_CLOSE_RATIO": {
            "type": "float", "default": 0.35, "min": 0.15, "max": 0.50,
            "description": "Макс. (close-low)/(high-low) для медвежьего закрытия свечи",
            "optimize": False,
        },
        "MAX_IMPULSE_STREAK": {
            "type": "int", "default": 3, "min": 2, "max": 6,
            "description": "Блокировка входа если N импульсных свечей из IMPULSE_WINDOW последних (anti-flush)",
            "optimize": False,
        },
        "IMPULSE_WINDOW": {
            "type": "int", "default": 5, "min": 3, "max": 10,
            "description": "Размер rolling-окна для anti-flush (N из M последних свечей)",
            "optimize": False,
        },
        "MAX_HOLD_CANDLES": {
        ...
        "COOLDOWN_CANDLES": {
            ...
            "description": "Мин. свечей между сделками (cooldown после закрытия)",
            "optimize": False,
        },
        "TRADE_FRACTION": {
    ...
    def __init__(self, engine: BaseOrderEngine) -> None:
        ...
        self._adx_prev_close: Optional[float] = None  # отдельный prev_close для ADX TR
        ...
        # --- Anti-flush rolling window ---
        self._impulse_history: list[bool] = []  # rolling window: True = импульсная медвежья свеча
    ...
    async def on_candle(self, candle: "Candle") -> None:
        ...
        # === 2. Anti-flush rolling window (до guard, нужен для истории) ===
        ...
        self._impulse_history.append(is_impulse_bear)
        if len(self._impulse_history) > self.IMPULSE_WINDOW:
            self._impulse_history = self._impulse_history[-self.IMPULSE_WINDOW:]
        ...
        if position < 0 and self._entry_price is not None and self._entry_atr is not None:
            ...
            exit_sl   = high >= sl_price   # SL по high — intra-candle пробой стопа
            ...
            if exit_tp or exit_sl or exit_rsi or exit_time:
                reason_parts = []
                if exit_sl:   reason_parts.append(f"SL(high={high:.2f}≥{sl_price:.2f})")
                ...
                # SL ВСЕГДА приоритетнее TP: на реальной бирже стоп сработает
                # раньше (цена сначала поднимается к SL, потом может упасть к TP).
                # Если SL сработал — закрываем по SL price, иначе по close.
                exit_price = sl_price if exit_sl else close
                await self._close_position(exit_price, "BUY", reason)
                self.logger.info(
                    f"CLOSE SHORT @ {exit_price:.2f}  {reason}  "
        ...
        if position == 0:
            ...
            # --- Cooldown check ---
            candles_since_last = self._candle_count - self._last_trade_candle
            if candles_since_last < self.COOLDOWN_CANDLES:
                self._append_history(high, low)
                return
            ...
            di_bear = (
                self._di_minus is not None
                and self._di_plus is not None
                and self._di_minus > self._di_plus
            )
            trend_ok = (
                close < self._ema_slow
                and self._ema_fast < self._ema_slow
                and self._adx > self.ADX_MIN
                and di_bear                          # DI- > DI+ подтверждает SHORT направление
                and vol_ratio > self.VOL_RATIO_MIN   # фильтр мёртвого рынка
            )
            ...
            if trend_ok:
                ...
                bear_close_ok = (
                    (close - low) / candle_range < self.BEAR_CLOSE_RATIO
                    if candle_range > 1e-9 else False
                )

                signal = breakout_ok and volume_ok and atr_size_ok and bear_close_ok

                # --- Anti-flush защита (rolling window: N of M) ---
                impulse_count = sum(self._impulse_history)
                flush_guard = impulse_count >= self.MAX_IMPULSE_STREAK
                ...
                if signal and not flush_guard:
                    result = await self._open_position(
                        ...
                        flush=f"{impulse_count}/{self.IMPULSE_WINDOW}",
                    )
                    if result is not None:
                        ...
                        self.logger.info(
                            ...
                            f"flush={impulse_count}/{self.IMPULSE_WINDOW}  "
    ...
    def _update_ema_slow(self, close: float) -> None:
        ...
        if self._ema_slow is None:
            ...
            if len(self._ema_slow_warmup) >= self.EMA_SLOW_PERIOD:
    ...
    def _update_adx(self, high: float, low: float, close: float) -> None:
        ...
        if self._adx_prev_high is None:
            ...
            self._adx_prev_close = close
            return

        prev_high  = self._adx_prev_high
        prev_low   = self._adx_prev_low
        prev_close = self._adx_prev_close if self._adx_prev_close is not None else close

        # True Range (uses ADX's own prev_close — not shared with ATR)
